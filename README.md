...
<p align="center">
  <img src="docs/screenshots/days.png" width="330" alt="跨过午夜：消息上方一条居中的日期，写成「昨天」「今天」或一个具体日期">
</p>

**一段对话可以横跨好几天，而面板一次只显示 60 行。** 这台机器上最长的会话跨 **37.9 小时**，最大的三个
会话各自跨越**三个日历天**——都远多于 60 行。此前窗口最上面那条消息只告诉你「有一阵子了」，却分不清
是二十分钟前还是两天前。现在每当相邻两条消息不在同一个日历天，上面那条就带一个日期标记。

判据是**日历天**而不是二十四小时：从 23:50 说到 00:10 跨了天（二十分钟），从 09:00 说到 20:00 没跨
（十一小时）。按小时算会把这两种情况都判反，而读者问的是「这是不是昨天」，不是「过了多久」。标记写成
「今天」「昨天」或一个具体日期——**能不用查日历就说出口的只有那两天**，其余的直接写日期，用的还是会话
列表里同一个 `calendarDate`，一个读者看到一种写法。
...
<p align="center">
  <img src="docs/screenshots/session-search.png" width="330" alt="按对话正文找会话：每行是会话名加两行摘要">
</p>

**你记得的是一句话，不是标题。** 会话列表此前只能按标题过滤，而标题是从一段对话的开头几个字推出来
的——所以记得「那次讨论 sqlite 索引的会话」的人，在列表里没有任何东西可输入。实测：`打造`（标题里有）
命中 **16** 行，而 `zstdDecompressSync`、`sqlite`、`PAGE_ROWS`、`attachmentId`（都出现在真实转写正文里）
命中 **0** 行。过滤是好的，只是看错了地方。

答案来自 harness 自己的全文索引（`ctx.sessionQuery`，官方侧栏的搜索也走它），所以这是**接端口而不是
再建一个索引**。命中排在标题之后而不是之前，因为两种答案性质不同：标题命中是「你叫得出名字的那个
会话」，正文命中是「你只记得内容」的那个。每行的摘要是标题承载不了的部分——它说明**为什么是这一个**。
「搜不到」与「这个配置没有索引」用两句话分开说：两者在屏幕上是同一片空白，而只有一种是「你的历史里
没有这个词」。
...
**已自动化验证**：上面五层测试，750 条。包括真实 Chrome 驱动的快照、点击、输入、截图，
...
| ★ 判据错在方向上：问「滚到最右时露出多少」只对右边缘的元素成立（v127） | 左边的块会被滚出左视野，于是「轻松可达」的块报成 0 露出。正确的问法是「**存在某个滚动位置**让它完整可见」——`L + W - C <= M` 且 `L >= 0` |
| ★ 造不出来的用例说明的是浏览器，不是判据（v127） | 想造「有横滚祖先但滚不到」的元素，两次都失败：`scrollWidth` 会把溢出内容算进去，`maximum` 恰好等于让右边缘可见所需的量。**两者永远相等**——这不是判据松，是浏览器就这么工作 |
| ★ 断言一个名字而不是断言工作（v127） | 变异把**调用**注释掉而留下定义，套件保持绿——我的断言只查了「规则存在」。断言 `foo(` 存在与断言 `foo(x)` 被调用是两件事 |
| ★ 把「可关闭」绑在 role 上会自相矛盾（v127） | 同一个浮层在「目录可用」态有 `role="menu"`、在「不可用」态故意设 `role="none"`（否则读屏会播报一份不存在的选项列表），于是审计对一个报红、对另一个报绿。**可关闭是行为，不是角色** |
| ★ 按 id 特判是判据腐烂的第一步（v127） | 我中途按 `#at-menu` 特判过，那是错的；改成通用判据（任何输入框里存在未结束的 `@`）。本仓库已记录过多次「按属性枚举，不按元素 id」 |
| ★ 判据「剥掉冒号前的部分看剩不剩东西」会把一句断掉的话读成完整的（v123） | 缺陷产出的菜单句子根本没有冒号，于是什么都剥不掉、剩下整句，探针报「有理由」。改成显式找分隔符，找不到就是没有理由。同一个形状的第五次：**一个不可能失败的判据读起来像通过** |
| ★ 给 `new URL` 传基址，会把相对路径读成绝对（v124） | `isSafeHref` 用 `new URL(href, 'https://example.invalid/')` 读协议，于是 `/docs/…` 拿到 `https:` 并通过检查——**解析正是让它看起来绝对的那个动作**。写进 `href` 后浏览器拿文档的基址（侧栏里是扩展自己的源）再解析一次。真实数据里这种链接有 258 个。官方 `sanitizeUrl` 用单参构造，相对路径抛错落进 `catch` |
| ★ 行内正则漏掉前缀字符，会把一种语法读成另一种（v125） | `INLINE_PATTERN` 从 `[` 开始匹配，于是 `![alt](src)` 里的 `!` 留在原地当文字、整张图变成链接。源是 https 时 `isSafeHref` 会通过，读者点下去到达一张 PNG——**29.6% 的真实图片是这样**。判据：一个语法记号如果由多个字符组成，正则必须从它的**第一个字符**开始匹配 |
| ★ 工具搬了家，留在原地的旧副本会继续被跑（v126） | `audit-all.mjs` 用 `join(HERE, 'preview.mjs')` 定位预览工具，而 v82 搬家时在 `.tmp-run/` 留下了一份 68 KB 的旧副本（真身 127 KB）。于是此后每一轮审计都在测**二十一轮之前**的工具，并报出一个产品不存在的缺陷（`.working` 对比度 1）。**它不会失败——一个跑错二进制的运行器与产品缺陷无法区分。** 修法是让两者住在同一目录 |
| ★ 系统色关键字脱离文档会被解析成默认值（v126） | `getComputedStyle(root).getPropertyValue('--token')` 返回**未解析**的字符串。把它画到 canvas 上求值，`CanvasText` 会按 canvas 自己的默认（浅色=黑）解析——量到的是 canvas 的上下文，不是页面的。由此得出「两种配色下同一个值」的假结论。**要在真实元素上读计算样式** |
| ★ 通知要按「读者是否需要据以行动」分成两种（v121） | 发送失败此前只用 toast 报告，而 toast 六秒后自清——实测提示消失后，屏幕上与「打好了字还没按发送」**完全一致**，而那正是唯一错误的读法。同一个陷阱 v102 已经踩过一次（旧宿主只用 toast 通知）。**「发生了某事」用 toast，「某事仍然成立」用常驻通知**；常驻通知由真正解决了它的动作清除，按键不算（改字是在处理，不是处理完了） |
| ★ 失败不能显示成事实（v122） | `refreshTranscript` 的判据是 `status === 0`，所以 **HTTP 500 落到 `Array.isArray(payload?.messages) ? … : []`，被画成「这个会话有零行」**——「宿主读不出来」与「会话是空的」在屏幕上一模一样，而且轮询每 8 秒重演一次。修法是 `if (!ok) { …; return }`：**屏幕留着宿主最后一次真正说过的话**，用空列表覆盖是断言一件刚被告知无法知道的事。注意 `return` 而不 `drawTranscript([])` 是承重部分 |
| ★ 断言「有字」不等于断言「是对的字」（v120） | 会话命中的标题只断言了「非空」，于是把标题换成那条会话的 id（一串不透明字符串）让套件**全绿**。变异脚本指出后果后才能补上 |
| ★ 把「没变化」与「不需要变」混为一谈（v119） | 日期标记写成 `data-day` 属性而不是插入节点，因为 `reconcileRows` 依赖「行节点是子节点前缀」这条不变量。一个插在行之间的小节会落进那个前缀，让按序放置的循环对到错的子节点 |
