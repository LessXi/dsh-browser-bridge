...
>
> 换了一条轴：**对比度的量化**。工具在 `%TEMP%\panel-preview\cdp-contrast2.mjs`
> —— 用 **canvas 把 CSS 颜色解析成 sRGB**（`ctx.fillStyle = css; fillRect; getImageData`）
> 再算 WCAG 比值。**这是关键**：这个面板的淡色全是
> `color-mix(in oklab, CanvasText N%, transparent)`，`getComputedStyle` 返回的是
> **`oklab(0 0 0 / 0.45)` 而不是 `rgb()`** ⇒ 只认 `rgb()` 的解析器会**静默跳过每一个**，
> 报出一份「全部通过」而其实**什么都没测**（第一版工具就是这么错的，报「9 处、0 失败」）。
>
> **实测（修复前）**：chat 浅色 43 处文字 **14 处低于 AA**；history 浅色 32 处 **13 处**。
> 全是 `--faint`(45%) 与 `--tertiary`(50%)，实测 **3.36 / 4.0**（需 4.5）。
> 而它们承载真内容：会话时间戳、工作区名、代码语言标签、**复制按钮的文字**、工具参数。
>
> **修法（按实测值）**：`--faint` 45%→**55%**（浅 3.36→4.76，深 4.46→6.00）；
> `--tertiary` 50%→**60%**（浅 3.98→5.74，深 5.19→6.89）。
>
> **实测（修复后）**：chat 43 / history 32 / approval 19 / long 24 处，**两主题全部 0 处低于 AA**。
> 唯一跳过的是 `.working` 的渐变文字（`background-clip: text` + `color: transparent`，
> 正常写法且已有 `prefers-reduced-motion` 回退）—— **不是缺陷**。
>
> **测试 444 条**（443→444）：守卫从 HTML 读百分比 + Chrome 的两个 Canvas 值算比值，
> 两主题都必须 ≥4.5（算的是近似，45% 时算式 3.35 vs 浏览器 3.36，偏差方向安全）。
> **证伪**：`--faint` 调回 45% → 1 红。
>
