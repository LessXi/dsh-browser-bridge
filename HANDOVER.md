...
所以记得「那次讨论 sqlite 索引的会话」的人，在列表里没有任何东西可输入。

### 读数：过滤是好的，只是看错了地方

`.tmp-run/probe-session-search-reach.js` 分两问量同一件事——先搜一个标题里确实有的词，
再搜几个几乎不可能出现在任何标题里、但一定在会话正文里的词：

| 查询 | 命中的行 |
| --- | --- |
| `打造`（标题里有） | **16** |
| `zstdDecompressSync` | **0** |
| `sqlite` | **0** |
| `PAGE_ROWS` | **0** |
| `attachmentId` | **0** |

标题能找到、正文一个都找不到，这不是管道坏了——**是搜索范围只覆盖标题**。
所以判据里那条 `findsByTitle` 是承重的：没有它，四个 0 可以被读成「过滤功能整个坏了」。

### ★ 交付方式：接上 harness 自己的索引，而不是再建一个

目标提到的「先考虑开源方案」在这里正好适用：harness 自带 `ctx.sessionQuery`，是一个
SQLite 支撑的跨会话全文检索服务，带 snippet、排名与分页游标——**官方侧栏的搜索就走它**。
所以这一轮是**接端口**而不是造轮子：在同一批日志上再建一个索引，是更差的答案，而且会漂移。

接入用 `ctx.get?.('sessionQuery')`，与 `sessions`/`workspaces`/`attachments` 同一种写法。
既有注释已经写明为什么必须**按调用时解析**：这些服务在插件激活时还没起来，
在激活时读一次会把 `undefined` 永久冻结。

### 「搜不到」与「没有可搜的东西」必须分开

服务是可选的——有的 profile 不挂载它。而**两者在屏幕上是同一片空白**，
只有一种是「你的历史里没有这个词」。所以 `searchEverySession` 返回 `available`，
面板据此画三句不同的话：找到 0 条 / 正在搜 / 这个配置没有会话检索。
`reason` 写成给读者看的句子，**不含服务名**（有测试断言这一点）。

### ★ 界面形态：命中画在标题之后，不是之前

两种答案是不同性质的：标题命中是「你叫得出名字的那个会话」，正文命中是「你只记得内容」的那个。
把命中排在前面，会让读者真正在找的那个会话掉到只是提到这个词的会话下面。

每行：会话名 + 两行以内的摘要（`-webkit-line-clamp: 2`）。摘要封顶是有意的——
它是通往一个会话的路标，不是阅读本身；不封顶时，一处命中落在一段长文里就能占满整屏。
一条没有摘要的命中**仍然成行**：会话是真实答案，缺摘要不改这一点。

### ★ 变异 9/9，但第一轮 7/9，暴露两个真实测试缺口

`.tmp-run/mutate-session-search.mjs`（用仓库自己的 `tools/mutate.mjs`）：

| 第一轮没被抓住的变异 | 缺什么 |
| --- | --- |
| `the-host-is-never-asked` | `entry.test.js` 只断言 `/browser-bridge/chat` **路由存在**，从没驱动过它的分发 |
| `the-set-query-fallback-renders-a-button` | 只断言了标题「非空」，没断言它**是那个会话的名字** |

第二条尤其值得记：把标题换成 `hit.sessionId` 会让套件**全绿**，而那时读者看到的是一串不透明 id。
**断言「有字」和断言「是对的字」是两件事。**

补上之后 9/9：7 条真坏法全部命中，1 条等价变异（把 `=== undefined ||` 改写成等价的否定式）
保持绿，文件全部复原。

### 一个自己的失误：数组断言撞上共享面板

第一版断言写的是 `deepEqual(host.sessionSearches.map(...), ['zstdDecompressSync'])`，
而 `panel-stream` 的测试**共享同一个面板实例**，数组里累积了先前测试输入过的词，于是报出
`['another', 'needle', 'number 1', 'zstdDecompressSync']`。
这不是产品缺陷，是断言没有隔离；改成「**包含**这个词」并且断言里带上实际数组，失败时读得出原因。

### 验证读数

- `npm test` → **738 passed, 0 failed, 0 skipped**（733 → 738，新增 5 条）
- `npm run check:extension` → exit 0
- 变异 **9/9** 符合声明，`restoredExactly` 通过
- 真实浏览器（场景 `sessionSearch`）：标题过滤 4 → **1**（既有行为未被破坏）、
  正文词找到 **3 个会话**、**2 条摘要**、无摘要那条仍成行、清空后完整列表复原
- 画廊 20 张，新增 `docs/screenshots/session-search.png`；
  `search.png` 出现已记录的焦点环抗锯齿抖动（1 像素，CSS `(7, 56.5)`），已 `git checkout` 还原

### 新增文件

- 仓库：`tools/showcase-session-search.js`（截图用探针，留在搜索状态）
- `.tmp-run/`（被 gitignore）：`probe-session-search-reach.js`（判据本体，两问对照）、
  `probe-session-search.js`（三问：标题仍工作 / 正文能搜到 / 清空复原）、`mutate-session-search.mjs`
